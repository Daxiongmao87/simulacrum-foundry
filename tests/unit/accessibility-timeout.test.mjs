...
test('e2e fixtures do not undercut the accessibility timeout budget', async () => {
  const fixtures = await readFile(join(process.cwd(), 'tests', 'e2e', 'fixtures', 'test-base.mjs'), 'utf8');
  assert.match(fixtures, /gamePage:[\s\S]*\{\s*timeout:\s*420000\s*\}/u);
  assert.match(fixtures, /simulacrumPage:[\s\S]*\{\s*timeout:\s*420000\s*\}/u);
});
