...
async function debugSimpleModal() {
  ...
  try {
    execSync(`docker build -f tests/docker/Dockerfile.foundry --build-arg FOUNDRY_VERSION_ZIP=FoundryVTT-Node-13.347.zip --build-arg FOUNDRY_MAIN_JS_PATH=/app/main.js --build-arg FOUNDRY_LICENSE_KEY=${foundryLicenseKey} -t ${imageName} .`, { stdio: 'inherit' });
