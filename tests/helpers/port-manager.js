...
export class PortManager {
  constructor(config) {
    ...
    console.log(`PortManager initialized: ${this.availablePorts.length} ports available (${this.portRange.start}-${this.portRange.end}), max ${this.maxConcurrentInstances} concurrent instances`);
  }
  ...
  async allocatePort(containerId) {
    ...
    if (this.allocatedPorts.size >= this.maxConcurrentInstances) {
      console.log(`Max concurrent instances (${this.maxConcurrentInstances}) reached. Queuing ${containerId}...`);
    ...
    if (!availablePort) {
      console.log(`No available ports in range ${this.portRange.start}-${this.portRange.end}. Queuing ${containerId}...`);
      return await this.queuePortRequest(containerId);
    }
    ...
    console.log(`Port ${availablePort} allocated to ${containerId} (${this.allocatedPorts.size}/${this.maxConcurrentInstances} instances)`);
  ...
  releasePort(containerId, port) {
    if (this.allocatedPorts.has(port)) {
      ...
      if (allocation.containerId === containerId) {
        ...
        console.log(`Port ${port} released by ${containerId} (${this.allocatedPorts.size}/${this.maxConcurrentInstances} instances)`);
  ...
  async queuePortRequest(containerId) {
    return new Promise((resolve, reject) => {
      ...
      console.log(`${containerId} queued for port allocation (position ${this.portQueue.length})`);
  ...
  processPortQueue() {
    ...
    console.log(`Port ${availablePort} allocated to queued ${request.containerId} (${this.allocatedPorts.size}/${this.maxConcurrentInstances} instances)`);
