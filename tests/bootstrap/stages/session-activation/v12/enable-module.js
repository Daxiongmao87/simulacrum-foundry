...
export class EnableModuleV12 {
  ...
  async enableModule(page, config) {
    try {
      console.log('Simulacrum | [V12 Module] 🚀 Starting module enable process...');
      
      // Open Settings sidebar tab
      console.log('Simulacrum | [V12 Module] 📍 Opening Settings sidebar tab...');
      await page.waitForSelector('#sidebar-tabs a[data-tab="settings"]', { timeout: 20000 });
      console.log('Simulacrum | [V12 Module] ✅ Settings tab selector found');
      
      await page.evaluate(() => {
        const tab = document.querySelector('#sidebar-tabs a[data-tab="settings"]');
        (tab instanceof HTMLElement) && tab.click();
      });
      console.log('Simulacrum | [V12 Module] ✅ Settings tab clicked');
      
      await page.waitForSelector('#settings', { timeout: 20000 });
      console.log('Simulacrum | [V12 Module] ✅ Settings panel loaded');

      // Click "Manage Modules"
      console.log('Simulacrum | [V12 Module] 📍 Clicking Manage Modules...');
      await page.waitForSelector('#settings button[data-action="modules"]', { timeout: 20000 });
      console.log('Simulacrum | [V12 Module] ✅ Manage Modules button found');
      
      await page.evaluate(() => {
        const btn = document.querySelector('#settings button[data-action="modules"]');
        (btn instanceof HTMLElement) && btn.click();
      });
      console.log('Simulacrum | [V12 Module] ✅ Manage Modules button clicked');

      // Wait for Module Management window
      console.log('Simulacrum | [V12 Module] 📍 Waiting for Module Management window...');
      await page.waitForSelector('#module-management', { timeout: 30000 });
      console.log('Simulacrum | [V12 Module] ✅ Module Management window loaded');
      
      await page.waitForSelector('#module-management #module-list', { timeout: 30000 });
      console.log('Simulacrum | [V12 Module] ✅ Module list loaded');

      // Filter to simulacrum to be safe
      console.log('Simulacrum | [V12 Module] 🔎 Filtering for simulacrum module...');
      ...
      console.log('Simulacrum | [V12 Module] ✅ Filter applied:', filterResult);

      // Check the simulacrum module checkbox
      console.log('Simulacrum | [V12 Module] 📍 Enabling Simulacrum module checkbox...');
      const moduleEnabled = await page.evaluate(() => {
        const row = document.querySelector('#module-management [data-module-id="simulacrum"]');
        console.log('Simulacrum | [V12 Module] 🔍 Looking for module row:', row);
        
        if (!row) {
          // Try alternative selectors
          const altRow = document.querySelector('#module-management tr:has(input[name="simulacrum"])');
          console.log('Simulacrum | [V12 Module] 🔍 Alternative row selector:', altRow);
          
          if (altRow) {
            const checkbox = altRow.querySelector('input.active[name="simulacrum"]');
            console.log('Simulacrum | [V12 Module] 🔍 Found checkbox via alt selector:', checkbox);
            if (checkbox) {
          ...
          console.log('Simulacrum | [V12 Module] 🔍 All available modules:', allModules);
          
          return { found: false, availableModules: allModules };
        }
        
        const checkbox = row.querySelector('input.active[name="simulacrum"]');
        console.log('Simulacrum | [V12 Module] 🔍 Found checkbox via primary selector:', checkbox);
        
        if (!checkbox) {
          // Try to find any checkbox in the row
          const anyCheckbox = row.querySelector('input[type="checkbox"]');
          console.log('Simulacrum | [V12 Module] 🔍 Any checkbox in row:', anyCheckbox);
          if (anyCheckbox) {
      ...
      console.log('Simulacrum | [V12 Module] ✅ Module enable attempt result:', moduleEnabled);
      ...
      console.log('Simulacrum | [V12 Module] 💾 Saving module settings...');
      const saveClicked = await page.evaluate(() => {
        const btn = document.querySelector('#module-management button[type="submit"]');
        console.log('Simulacrum | [V12 Module] 🔍 Save button found:', btn);
      ...
      console.log('Simulacrum | [V12 Module] ✅ Save button clicked:', saveClicked);
      
      if (!saveClicked) throw new Error('Save Module Settings button not found');

      // Reload confirmation is mandatory in v12
      console.log('Simulacrum | [V12 Module] 🔁 Waiting for reload confirmation dialog...');
      ...
      console.log('Simulacrum | [V12 Module] ✅ Reload confirmation dialog appeared:', confirmAppeared);
      
      if (!confirmAppeared) {
        throw new Error('Reload confirmation dialog not detected');
      }

      console.log('Simulacrum | [V12 Module] 📍 Confirming reload (Yes)...');
      const clickedYes = await page.evaluate(() => {
        ...
        console.log('Simulacrum | [V12 Module] 🔍 Yes button found:', yesBtn);
      ...
      console.log('Simulacrum | [V12 Module] ✅ Yes button clicked:', clickedYes);
      ...
      console.log('Simulacrum | [V12 Module] ⏳ Waiting for FoundryVTT to reinitialize after module activation...');
      const foundryLoadedPromise = new Promise((resolve) => {
        const listener = (msg) => {
          try {
            const text = msg.text();
            console.log('Simulacrum | [V12 Module] 📝 Console message:', text);
            if (text.includes('Foundry VTT | Drawing game canvas for scene')) {
              console.log('Simulacrum | [V12 Module] ✅ FoundryVTT game canvas ready - full initialization complete');
        ...
        setTimeout(() => { 
          console.log('Simulacrum | [V12 Module] ⏰ Timeout reached for FoundryVTT initialization');
          page.off('console', listener); 
          resolve(); 
        }, 30000);
      });
      ...
      console.log('Simulacrum | [V12 Module] 📍 Waiting 10 seconds grace period for full initialization...');
      await new Promise(resolve => setTimeout(resolve, 10000));

      console.log('Simulacrum | [V12 Module] ✅ Module activation reload complete (mandatory-confirm)');
      return { success: true };
    } catch (error) {
