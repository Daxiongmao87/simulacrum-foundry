    ...
    try {
      ...
      if (eulaCheck.detected) {
        ...
        if (result.success) {
          ...
          console.log(`[V12 EULA] ⚠️ EULA acceptance failed: ${result.reason}`);
          return result;
        }
      } else {
        console.log('[V12 EULA] 📍 No EULA detected, continuing to setup...');
        return { success: true, method: 'no_eula' };
      }
      
    } catch (error) {
      console.log(`[V12 EULA] ⚠️ EULA check failed: ${error.message}, continuing...`);
  ...
  async handleEULAOnSetupPage(page, config) {
    ...
    try {
      ...
      if (eulaResult.success) {
        ...
        console.log(`[V12 EULA] ⚠️ EULA acceptance failed: ${eulaResult.error}`);
        return { success: false, error: eulaResult.error };
      }
    } catch (e) {
      console.log('[V12 EULA] 📍 No EULA appeared on setup page, continuing...');
