...
import { pollUntil, pollForElement, pollUntilGone, waitForUiSettle } from './poll-utils.mjs';
...
export async function loginAsAdmin(page, adminKey, baseUrl = '') {
  ...
  if (currentUrl.includes('/join')) {
    console.log(
      '[helper] loginAsAdmin: World is running (on /join). Will let launchWorld handle this.'
    );
  ...
  const adminKeyInput = page.locator(
    'input[name="adminKey"], input[name="adminPassword"], #admin-password'
  );
...
/**
 * Complete Foundry's first-launch User Management gate when it appears.
 * Foundry v14 routes a newly created world through /players before the game UI.
 *
 * @param {import('@playwright/test').Page} page
 * @param {object} [options]
 * @param {string} [options.logPrefix]
 * @returns {Promise<boolean>} true when the gate was present and submitted
 */
export async function completeUserManagementIfPresent(page, { logPrefix = 'helper' } = {}) {
  const playersPath = /\/players(?:$|[/?#])/;
  const managementForm = page.locator('form#manage-players, #manage-players').first();

  const onPlayersPage = playersPath.test(page.url());
  const formVisible = await managementForm
    .isVisible({ timeout: onPlayersPage ? 30000 : 1000 })
    .catch(() => false);

  if (!onPlayersPage && !formVisible) return false;

  console.log(`[${logPrefix}] Completing Foundry user-management gate`);

  const submitButton = managementForm
    .locator(
      'button[type="submit"], button:has-text("Save and Continue"), button:has-text("Launch")'
    )
    .first();
  await expect(submitButton).toBeVisible({ timeout: 30000 });
  await submitButton.click();

  await page.waitForURL(/\/(?:game|join)(?:$|[/?#])/, { timeout: 120000 });
...
export async function installSystem(page, systemId, baseUrl = '') {
  ...
  await pollForElement(page, `[data-package-id="${systemId}"], li:has-text("${systemId}")`, {
    timeout: 5000,
  }).catch(() => {});

  // Find and click the system in results
  const systemEntry = dialog
    .locator(`[data-package-id="${systemId}"], li:has-text("${systemId}")`)
    .first();
...
export async function enableModuleInWorld(page, worldId, moduleId, baseUrl = '') {
  ...
  const moduleCheckbox = page.locator(
    `input[name="modules.${moduleId}"], input[data-module="${moduleId}"]`
  );
...
export async function launchWorld(page, worldId, baseUrl = '') {
  ...
  if (currentUrl.includes('/players')) {
    const completed = await completeUserManagementIfPresent(page, { logPrefix: 'launchWorld' });
    if (completed && page.url().includes('/game')) {
      console.log(`[helper] launchWorld: User management completed and game is already loaded.`);
      return;
    }
  }
  ...
  if (currentUrl.includes('/join')) {
    console.log(
      `[helper] launchWorld: Already on /join page, world is already running. Skipping launch.`
    );
  ...
  if (urlAfterNav.includes('/players')) {
    const completed = await completeUserManagementIfPresent(page, { logPrefix: 'launchWorld' });
    if (completed && page.url().includes('/game')) {
      console.log(`[helper] launchWorld: User management completed and game is already loaded.`);
      return;
  ...
  if (urlAfterNav.includes('/join')) {
    console.log(
      `[helper] launchWorld: Redirected to /join, world is already running. Skipping launch.`
    );
    return;
  }
  ...
  if (await worldsTab.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    await page
      .locator('section[data-tab="worlds"], .worlds-list, [data-package-id]')
      .first()
  ...
  const buttonExists = await worldCard
    .locator('[data-action="worldLaunch"], a.control.play')
    .count();
  ...
  if (buttonExists === 0) {
    ...
    await pollForElement(page, '[data-action="worldLaunch"], a.control.play', {
      timeout: 2000,
    }).catch(() => {});
  }
  ...
  if (!worldLaunchTriggered) {
    ...
    await pollForElement(page, '[data-action="worldLaunch"], button:has-text("Launch")', {
      timeout: 5000,
    }).catch(() => {});
  }
  ...
  if (!worldLaunchTriggered) {
    if (!launchButton || !(await launchButton.isVisible({ timeout: 1000 }).catch(() => false))) {
      ...
      console.log(
        `[helper] No launch button found or not visible, trying double-click on world card...`
      );
...
export async function joinAsUser(page, userName, password = '') {
  ...
  const joinButton = page
    .locator('button[name="join"], button:has-text("Join Game Session"), button:has-text("Join")')
    .first();
...
export async function openModuleSettings(page, moduleId) {
  ...
  const settingsButton = page.locator(
    '#settings button:has-text("Configure Settings"), button:has-text("Configure Settings")'
  );
...
export async function enableModuleViaUI(page, moduleId = 'simulacrum') {
  ...
  try {
    ...
    if (!(await moduleRow.isVisible({ timeout: 2000 }).catch(() => false))) {
      ...
      const visibleModules = await moduleManagementDialog
        .locator('li[data-module-id]')
        .allInnerTexts();
      console.log(
        `[helper] Visible modules: ${visibleModules.map(t => t.substring(0, 50)).join(', ')}`
      );
      await page.screenshot({ path: '/tmp/module-management-debug.png' });
      return false;
    }
    ...
    if (!(await checkbox.isVisible({ timeout: 1000 }).catch(() => false))) {
      ...
      console.log(
        `[helper] Found module checkbox, isChecked=${isChecked}, isDisabled=${isDisabled}`
      );
      if (!isChecked) {
        if (isDisabled) {
        ...
        if (!nowChecked) {
          console.log(
            `[helper] WARNING: JS click did not work, trying direct property set + form change...`
          );
    ...
    if (await saveBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await page.evaluate(() => {
        const btn = document.querySelector(
          '#module-management footer.form-footer button[type="submit"]'
        );
        if (btn) {
          btn.click();
        }
      ...
      await pollUntil(
        async () => {
          ...
          const stillVisible = await moduleManagementDialog
          ...
          if (confirmVisible) {
            ...
            const confirmBtn = page
              .locator(
                '.dialog button:has-text("Yes"), .dialog button:has-text("Reload"), .dialog button[data-button="yes"]'
              )
              .first();
...
/**
 * Open the Simulacrum sidebar tab through Foundry's native sidebar controls.
 * @param {import('@playwright/test').Page} page
 */
export async function openSimulacrumSidebar(page) {
  const tabButton = page
    .locator(
      '#sidebar-tabs [data-tab="simulacrum"], ' +
        'nav.tabs [data-tab="simulacrum"], ' +
        'button[data-tab="simulacrum"]'
    )
    .first();
  ...
  await page.waitForFunction(
    () => {
      ...
      return ui?.sidebar?.tabGroups?.primary === 'simulacrum';
    },
    null,
    { timeout: 10000 }
  );
  ...
  const tabContent = page
    .locator(
      '#sidebar-content #simulacrum, ' +
        '#sidebar-content section[data-tab="simulacrum"], ' +
        'section#simulacrum[data-tab="simulacrum"]'
    )
    .first();
