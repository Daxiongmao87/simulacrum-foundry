...
describe('DocumentUpdateTool - constructor', () => {
  ...
  it('should define required parameters correctly', () => {
    ...
    // expect(documentType.required).toBe(true); // JSON Schema uses required array
    expect(documentType.description).toContain('The type/class of document');
...
describe('DocumentUpdateTool - Utility Methods', () => {
  ...
  describe('getConfirmationDetails', () => {
    it('should return correct confirmation details', async () => {
      ...
      expect(details.title).toBe('Update Actor');
      expect(details.html).toContain('Type:</strong> Actor');
      expect(details.html).toContain('ID:</strong> actor-123');
      expect(details.html).toContain('Updates:</strong><pre>{');
    });
    ...
    it('should handle complex update objects', async () => {
      ...
      // expect(details.type).toBe('update'); // type field removed
      expect(details.title).toBe('Update Item');
      expect(details.html).toContain('Updates:');
      expect(details.html).toContain('Magic Sword');
      // expect(details.html).toContain('operations'); // Only present if operations provided
    });
  });
});
...
describe('DocumentUpdateTool - Legacy Compatibility', () => {
  ...
  describe('execute error handling', () => {
    ...
    it('should return error for invalid document type', async () => {
      ...
      expect(result.display).toContain('Validation Error: Document type "InvalidType" not available');
