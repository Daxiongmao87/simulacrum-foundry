...
export default async function simulacrumInitTest(session, permutation, config) {
  ...
  try {
    ...
    // Wait up to 60s for the module ready hook to complete to avoid timing flakiness
    try {
      await page.waitForFunction(() => {
        const st = window.game?.simulacrum?._initState;
        return !!st && st.initComplete === true && st.readyComplete === true;
      }, { timeout: 60000 });
    } catch (_) {
      // continue to collect the current state for assertion output
    }
