...
async function debugCreation() {
  ...
  if (sys.description) {
    ...
    if (typeof sys.description === 'string') {
      console.log(
        'ISSUE: Description is a STRING. Foundry Schema expects OBJECT { value: string }.'
      );
      console.log(
        '       Result: Foundry will likely ignore/drop this value or throw validation error.'
      );
    }
  } else {
  ...
  if (sys.abilities) {
    ...
    console.log(
      'ISSUE: Abilities NOT in system (likely rejected because "abilities" is not in schema fields).'
    );
    console.log(
      '       Available mock system fields:',
      Object.keys(mockSchema.fields.system.fields)
    );
  }
}
