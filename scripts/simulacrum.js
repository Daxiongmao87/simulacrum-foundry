...
async function ensureSimulacrumSidebarRendered() {
  if (!ui.simulacrum && CONFIG?.ui?.simulacrum) {
  ...
  try {
    ...
    logger.info('Rendered Simulacrum sidebar tab after Foundry sidebar initialization');
  } catch (error) {
    logger.error(
      'Failed to render Simulacrum sidebar tab after Foundry sidebar initialization',
      error
    );
  }
}

function scheduleSimulacrumSidebarRender() {
  window.setTimeout(() => {
    void ensureSimulacrumSidebarRendered();
  }, 0);
}
