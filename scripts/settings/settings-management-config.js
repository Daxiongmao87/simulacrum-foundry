...
export class SettingsManagementConfig extends FormApplication {
    ...
    activateListeners(html) {
        ...
        html.find('.test-connection-button').on('click', this._onTestConnection.bind(this));
        html.find('.export-settings-button').on('click', this._onExportSettings.bind(this));
        html.find('.import-settings-button').on('click', this._onImportSettings.bind(this));
    }
    ...
    async _onTestConnection(event) {
        ...
        try {
            ...
            if (response.ok) {
                ui.notifications.info(game.i18n.localize("SIMULACRUM.ConnectionSuccess"));
                button.html('<i class="fas fa-check"></i> Success').addClass('success');
            } else {
                const errorData = await response.json();
                ui.notifications.error(game.i18n.format("SIMULACRUM.ConnectionFailed", { status: response.status, message: errorData.error.message || response.statusText }));
                button.html('<i class="fas fa-times"></i> Failed').addClass('failure');
            }
        } catch (error) {
            console.error("Simulacrum | API Connection Test Error:", error);
            ui.notifications.error(game.i18n.format("SIMULACRUM.ConnectionError", { message: error.message }));
            button.html('<i class="fas fa-times"></i> Error').addClass('failure');
        } finally {
            setTimeout(() => {
                button.prop('disabled', false).html(`${originalIcon} ${originalText}`).removeClass('success failure');
            }, 2000);
        }
    }
    ...
    _onExportSettings(event) {
        ...
        saveDataToFile(JSON.stringify(settings, null, 2), 'application/json', filename);
            ...
            callback: async (path) => {
                try {
                    ...
                    if (typeof importedSettings !== 'object' || importedSettings === null) {
                        throw new Error(game.i18n.localize("SIMULACRUM.ImportInvalidFormat"));
                    }
                    ...
                    for (const key in importedSettings) {
                        if (game.settings.settings.has(`simulacrum.${key}`)) {
                            ...
                            if (setting.scope === 'world' && setting.module === 'simulacrum') {
                                await game.settings.set('simulacrum', key, importedSettings[key]);
                            }
                        }
                    }
                    ui.notifications.info(game.i18n.localize("SIMULACRUM.SettingsImported"));
                    this.render(true); // Re-render to show updated settings
                } catch (error) {
                    console.error("Simulacrum | Settings Import Error:", error);
                    ui.notifications.error(game.i18n.format("SIMULACRUM.ImportFailed", { message: error.message }));
                }
            },
