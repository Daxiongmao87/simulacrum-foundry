...
export class SimulacrumChatModal {
  ...
  constructor(options = {}) {
    ...
    // Listen for Gremlin mode changes to update UI
    Hooks.on('updateSetting', (setting) => {
      if (setting.key === 'simulacrum.gremlinMode') {
        this._updateGremlinIndicator();
      }
    });
    ...
    // const originalSendMethod = this.chatWindow._onSendMessage; // Stored but not used - method is directly overridden
  ...
  _cancelCurrentOperation() {
    ...
    this.chatWindow.addMessage({
      content:
        '<p class="simulacrum-system"><i>Request cancelled by user</i></p>',
      sender: 'System',
  ...
  _setupContextContainerUI() {
    setTimeout(() => {
      let contextContainer = $(this.chatWindow.element).find(
      ...
      if (!contextContainer.length) {
        ...
        contextContainer = $(
          '<div class="auxiliary-content-container"><div class="simulacrum-context-items"></div></div>'
        );
  ...
  _updateContextItemsUI() {
    ...
    const auxiliaryContainer = $(this.chatWindow.element).find(
      '.auxiliary-content-container'
    );
  ...
  _setupCopyButtons() {
    ...
    setTimeout(() => {
      ...
      assistantMessages.each((i, message) => {
        ...
        const tooltip = $(`<div class="simulacrum-copy-tooltip">Copied!</div>`);
  ...
  _processElementForTextCopy(element) {
    ...
    links.forEach((link) => {
      if (
        link.href &&
        link.textContent &&
        !link.textContent.includes(link.href)
      ) {
  ...
  async _handleUserMessage(message) {
    try {
      ...
      if (!this.aiService) {
        this.aiService = game.simulacrum.aiService;
      }
   ...
   * @private
   */
  _displayToolResult(result, _messageElement) {
  ...
  addDocumentContext(document) {
    if (!document || !(document instanceof foundry.abstract.Document)) {
      ...
      ui.notifications.error(
        'Simulacrum | Failed to add document to context: Invalid document.'
      );
      return;
    }
    ...
    if (this.contextDocuments.some((d) => d.uuid === document.uuid)) {
      ui.notifications.warn(
        `Simulacrum | ${document.name} is already in context.`
      );
      return;
    }
