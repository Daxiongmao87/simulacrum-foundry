    ...
    async detectContextWindow(apiEndpoint, modelName) {
        try {
            ...
            if (cachedWindow && cachedWindow > 0) {
                console.log(`🔍 Using cached context window: ${cachedWindow} tokens for ${modelName}`);
                return cachedWindow;
            }
        ...
        console.log(`🔍 Using fallback context window: ${fallbackWindow} tokens for ${modelName || 'unknown model'}`);
...
export class ContextCompaction {
    ...
    async initialize(apiEndpoint, modelName) {
        this.maxTokens = await this.detector.detectContextWindow(apiEndpoint, modelName);
        console.log(`📦 Context compaction initialized with ${this.maxTokens} max tokens`);
    }
    ...
    async checkAndCompact(chatHistory, tokenTracker) {
        ...
        if (currentUsage > thresholdTokens) {
            console.log(`🗜️ Context compaction triggered at ${currentUsage}/${this.maxTokens} tokens (${Math.round((currentUsage/this.maxTokens)*100)}%)`);
    ...
    async performCompaction(chatHistory) {
        try {
            ...
            const hasSystemMessage = chatHistory.length > 0 && 
                (chatHistory[0].role === 'system' || chatHistory[0].role === 'assistant');
            const preservedStart = hasSystemMessage ? chatHistory.slice(0, 1) : [];
            const workingHistory = hasSystemMessage ? chatHistory.slice(1) : chatHistory;
            ...
            console.log(`📦 Compacting ${olderHalf.length} older messages, preserving ${newerHalf.length} newer messages`);
            ...
            console.log(`📦 Compacted ${olderHalf.length} messages into 1 summary. New history length: ${compactedHistory.length}`);
