...
export function registerDynamicSchemaModifier(documentDiscoveryEngine) {
  ...
  const createValidationHook = (documentType) => {
    return (document, data, _options, _userId) => {
      ...
      console.log(
        `Simulacrum | Schema has img field, proceeding with validation`
      );
      ...
      if (!imgPath || imgPath.trim() === '') {
        console.log(
          `Simulacrum | img field is missing or empty, blocking creation`
        );
  ...
  for (const docType of documentTypes) {
    ...
    console.log(
      `Simulacrum | ${hookName} hook registered with ID:`,
      hookResult
    );
  }
  ...
  const updateHookResult = Hooks.on(
    'preUpdateDocument',
    (document, updates, _options, _userId) => {
  ...
  console.log(
    'Simulacrum | preUpdateDocument hook registration result:',
    updateHookResult
  );

  console.log(
    'Simulacrum | Dynamic schema modifier hooks registered successfully'
  );
  console.log('Simulacrum | Registered hooks:', registeredHooks);
  console.log(
    'Simulacrum | preUpdateDocument hooks registered:',
    Hooks.events?.preUpdateDocument?.length || 0
  );
}
