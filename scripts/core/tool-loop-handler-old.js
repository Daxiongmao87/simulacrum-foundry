...
/**
 * Process tool call loop for pure tool execution
 * Returns execution results to ChatHandler for conversation management
 */
export async function processToolCallLoop(
  ...
  // Skip initial message handling if flagged
  if (!currentResponse._skipInitialMessage) {
    // Notify about initial response if not a parse error
    if (!currentResponse._parseError && onToolResult) {
      onToolResult({
        role: 'assistant',
        content: currentResponse.content,
        display: currentResponse.display || currentResponse.content
      });
    }
  }
  
  while (!endTaskSignaled && !guardTriggered && repeatCount < REPEAT_LIMIT) {
    // Check for cancellation
    if (signal?.aborted) {
      throw new Error('Process was cancelled');
    }
    
    // If no tool calls and no parse error, continue autonomous loop
    if ((!Array.isArray(currentResponse.toolCalls) || currentResponse.toolCalls.length === 0) && !currentResponse._parseError) {
      return await continueAutonomousLoop(currentResponse, conversationManager, aiClient, getSystemPrompt, signal, onToolResult);
    }
