...
export class TokenTracker {
    ...
    addPromptTokens(count) {
        if (typeof count !== 'number' || count < 0) {
            console.warn("Simulacrum | TokenTracker: Invalid prompt token count provided.", count);
            return;
        }
        this._promptTokens += count;
        this._totalTokens += count;
        console.log(`Simulacrum | TokenTracker: Added ${count} prompt tokens. Current total: ${this._promptTokens}`);
    }
    ...
    addCompletionTokens(count) {
        if (typeof count !== 'number' || count < 0) {
            console.warn("Simulacrum | TokenTracker: Invalid completion token count provided.", count);
            return;
        }
        this._completionTokens += count;
        this._totalTokens += count;
        console.log(`Simulacrum | TokenTracker: Added ${count} completion tokens. Current total: ${this._completionTokens}`);
    }
    ...
    setMaxTokens(maxTokens) {
        ...
        console.log(`🎯 TokenTracker initialized with ${maxTokens} token context window`);
    }
...
export function formatToolResultsForAI(toolResults, tokenTracker) {
    ...
    let formattedResults = toolResults.map(result => {
        if (result.success) {
    ...
    if (formattedResults.length > maxChars) {
        formattedResults = formattedResults.substring(0, maxChars) + "...[results truncated]";
    }
