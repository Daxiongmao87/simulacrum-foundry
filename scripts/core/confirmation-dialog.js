...
export class ToolConfirmationDialog extends Dialog {
    ...
    return new Promise((resolve) => {
      new Dialog({
        ...
        buttons: {
          proceedOnce: {
            label:
              game.i18n.localize('SIMULACRUM.ConfirmProceedOnce') ||
              'Proceed Once',
            callback: () => resolve('ProceedOnce'),
          },
          proceedAlways: {
            label:
              game.i18n.localize('SIMULACRUM.ConfirmProceedAlways') ||
              'Proceed Always',
  ...
  static async _assessImpact(toolName, parameters, _options) {
    switch (toolName) {
