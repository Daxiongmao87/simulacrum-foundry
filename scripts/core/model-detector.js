...
export class ModelDetector {
        ...
        try {
            ...
            for (const result of detectionResults) {
                if (result.status === 'fulfilled' && result.value.models.length > 0) {
                    console.log(`🎯 Found ${result.value.models.length} models via ${result.value.type}`);
            ...
            const errorResult = { type: 'error', models: [], detectable: false, error: error.message };
    ...
    async testOllamaModels(apiEndpoint) {
        ...
        const ollamaEndpoint = apiEndpoint.replace('/v1/chat/completions', '').replace('/v1', '');
    ...
    async getModelsForType(apiEndpoint, apiType, apiKey = null) {
        try {
            ...
            return { type: apiType, models: [], detectable: false, error: error.message };
        }
    }
