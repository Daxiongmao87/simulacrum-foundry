...
export class DocumentDiscoveryEngine {
  ...
  async getAvailableTypes() {
    ...
    for (const [name] of game.collections.entries()) {
      if (DocumentDiscoveryEngine.USER_CREATABLE_COLLECTIONS.includes(name)) {
        types[name] = { collection: name, isCollection: true };
      }
    }

    // Check subtypes via CONFIG (system-specific types discovered from active game system)
    for (const [docType, config] of Object.entries(window.CONFIG)) {
      if (
        config?.typeLabels &&
        DocumentDiscoveryEngine.USER_CREATABLE_COLLECTIONS.includes(docType)
      ) {
  ...
  async normalizeDocumentType(documentType) {
    ...
    if (!normalized) {
      throw new Error(
        `Simulacrum | Document type "${documentType}" not found or is not creatable.`
      );
    }
   ...
   */
  getAllDocumentTypes() {
    return DocumentDiscoveryEngine.USER_CREATABLE_COLLECTIONS.filter(
      (collectionName) => {
