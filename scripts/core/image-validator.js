...
export class ImageValidator {
    ...
    static async validateImagePath(imagePath, options = {}) {
        ...
        if (cachedResult && (now - cachedResult.timestamp < ImageValidator.#CACHE_DURATION)) {
            return { isValid: cachedResult.isValid, message: cachedResult.message || 'Cached result' };
        }

        if (required && (!imagePath || imagePath.trim() === '')) {
            const result = { isValid: false, message: 'Image path is required and cannot be empty.' };
            ImageValidator.#validationCache.set(cacheKey, { ...result, timestamp: now });
            return result;
        }

        if (!imagePath || imagePath.trim() === '') {
            const result = { isValid: true, message: 'Image path is optional and not provided.' };
            ImageValidator.#validationCache.set(cacheKey, { ...result, timestamp: now });
            return result;
        }
        ...
        try {
            const timeoutPromise = new Promise((_, reject) =>
                setTimeout(() => reject(new Error('Image path validation timed out after 30 seconds.')), 30000)
            );

            const validationPromise = (async () => {
                if (!ImageValidator.isValidImageFormat(imagePath)) {
                    return { isValid: false, message: `Invalid image format for path: ${imagePath}. Supported formats: .webp, .png, .jpg, .jpeg, .gif, .svg.` };
                }

                const exists = await ImageValidator.fileExists(imagePath);
                if (!exists) {
                    return { isValid: false, message: `Image file does not exist at path: ${imagePath}.` };
                }
            ...
            const result = { isValid: false, message: `Validation error for image path '${imagePath}': ${error.message}` };
            ImageValidator.#validationCache.set(cacheKey, { ...result, timestamp: now });
            return result;
        }
    }
    ...
    static async validateDocumentImages(documentData, documentType) {
        const errors = [];
        const imageFields = Object.keys(documentData).filter(ImageValidator.isImageField);
        ...
        for (const fieldName of imageFields) {
            ...
            const { isValid, message } = await ImageValidator.validateImagePath(imagePath, { required: isRequired });
            if (!isValid) {
                errors.push(`Document ${documentType} | Field '${fieldName}': ${message}`);
            }
        }
    ...
    static isImageField(fieldName) {
        const imageFieldNames = ['img', 'thumbnail', 'icon', 'portrait', 'tokenImg', 'avatar'];
    ...
    static async fileExists(path) {
        try {
            // FoundryVTT's FilePicker.browse can check for file existence.
            // It returns an object with 'files' array if the directory exists and contains files.
            // We need to extract the directory and filename from the path.
            const parts = path.split('/');
            const filename = parts.pop();
            const directory = parts.join('/');

            // Use FilePicker.browse to check if the file exists in the specified directory.
            // The source 'data' refers to the FoundryVTT user data directory.
            const browseResult = await FilePicker.browse('data', directory, { extensions: [] }); // No specific extensions needed for existence check

            return browseResult.files.includes(`${directory}/${filename}`);
        } catch (error) {
            console.error(`Simulacrum | ImageValidator | Error checking file existence for ${path}:`, error);
            // If FilePicker.browse throws an error (e.g., directory doesn't exist), assume file doesn't exist.
            return false;
        }
    }
