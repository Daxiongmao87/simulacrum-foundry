...
export async function buildSystemPrompt() {
  ...
  console.log(
    `%c[Simulacrum] System Prompt Verification (v${version})`,
    'color: #00ff00; font-weight: bold; font-size: 12px;'
  );
  console.log(`[Simulacrum] Content Length: ${basePrompt.length}`);
  if (isClean) {
    console.log(
      '%c[Simulacrum] Security Check: PASS (No forbidden HTML tags found)',
      'color: green'
    );
  } else {
    console.error(
      `[Simulacrum] Security Check: FAIL (Found tags: ${foundTags.slice(0, 10).join(', ')}${foundTags.length > 10 ? '...' : ''})`
    );
