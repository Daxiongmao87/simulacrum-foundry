...
export class ContextWindowDetector {
    ...
    async getContextWindow(apiEndpoint, modelName) {
        ...
        try {
            const showEndpoint = apiEndpoint.replace('/v1', '/api/show');
            console.log(`🔍 Getting context window for ${modelName} from ${showEndpoint}`);
            ...
            if (response.ok) {
                ...
                if (data.parameters?.num_ctx) {
                    ...
                    console.log(`🎯 Found context window in parameters: ${contextWindow}`);
                ...
                if (data.modelfile) {
                    ...
                    if (match) {
                        ...
                        console.log(`🎯 Found context window in modelfile: ${contextWindow}`);
                        return contextWindow;
                    }
                }
                ...
                if (data.model_info?.num_ctx) {
                    ...
                    console.log(`🎯 Found context window in model_info: ${contextWindow}`);
                    return contextWindow;
                }

                console.log(`⚠️ No context window found for ${modelName}, using default 8192`);
                return 8192;
            }

            console.warn(`⚠️ Failed to get model info for ${modelName}: HTTP ${response.status}`);
            return 8192;

        } catch (error) {
            console.warn(`⚠️ Context window detection failed for ${modelName}: ${error.message}`);
            return 8192;
        }
    }
