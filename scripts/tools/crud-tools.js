...
export class CreateDocumentTool extends Tool {
    ...
    super(
      ...
      'Creates a new FoundryVTT document of a specified type with provided data.',
      {
        ...
        properties: {
          documentType: {
            type: 'string',
            description: 'The type of document to create (e.g., "Actor", "character", "Item", "weapon").'
          },
          data: {
            type: 'object',
            description: 'The data to initialize the new document with (e.g., { name: "New Actor", img: "path/to/img.png" }).'
          }
        },
  ...
  async execute(params) {
    ...
    console.log("Simulacrum | Calling crudService.createDocument() with:", params.documentType, params.data);
    try {
      const result = await this.crudService.createDocument(params.documentType, params.data);
      console.log("Simulacrum | crudService.createDocument() succeeded:", result);
      return result;
    } catch (error) {
...
export class ReadDocumentTool extends Tool {
    ...
    super(
      ...
      'Reads a FoundryVTT document by its ID and type.',
      {
        ...
        properties: {
          documentType: {
            type: 'string',
            description: 'The type of document to read (e.g., "Actor", "Item", "Scene").'
          },
          documentId: {
            type: 'string',
   ...
   */
  async execute(params) {
    return this.crudService.readDocument(params.documentType, params.documentId);
  }
}
...
export class UpdateDocumentTool extends Tool {
   ...
   */
  async execute(params) {
    return this.crudService.updateDocument(params.documentType, params.documentId, params.updates);
  }
}
...
export class DeleteDocumentTool extends Tool {
   ...
   */
  async execute(params) {
    return this.crudService.deleteDocument(params.documentType, params.documentId);
  }
}
