...
export class DynamicContextWindowSetting {
    ...
    this.debounceTimer = setTimeout(async () => {
      try {
        ...
        if (game.simulacrum?.dynamicModelSelector) {
          await game.simulacrum.dynamicModelSelector.updateModelSelection(
            value,
            apiKey
          );
        }
  ...
  async onModelChange(modelName) {
    ...
    try {
      ...
      const contextWindow = await this.detector.getContextWindow(
        apiEndpoint,
        modelName
      );
  ...
  async showAutoDetectedField(_detection) {
    ...
    if (modelName && apiEndpoint) {
      try {
        detectedValue = await this.detector.getContextWindow(
          apiEndpoint,
          modelName
        );
      } catch {
  ...
  addOverrideCheckbox() {
    ...
    this.overrideCheckbox = this.settingElement.find(
      '.simulacrum-context-override'
    );
  ...
  showOverrideCheckbox() {
    const container = this.settingElement.find(
      '.simulacrum-override-container'
    );
    container.show();
  }
